...
write_csv(clean_main_index,"../data/mainindex_11_09.csv")
...
clean_newspaper_list <- newspaper_list %>% 
  clean_names() %>% 
  #i added the lccn, oclc and issn, which allow us to link to the Printing Hate index 
...
## Join test NA with the LOC main index by sn
```{r}
#20,226 record with no state
filtered_main_index <- clean_main_index %>% 
  filter(is.na(newspaper_state_clean)) 

#20,529 records with states added
#why did it add 303 records? 
...
#new index joins with clean newspaper
#62,814.
#why did it add 2,772 records?
...
x <- main_index_2 %>% 
  anti_join(new_main_index)



```

# MORE NEWSPAPER CLEANING
```{r}
state_crosswalk <- read_csv("../data/states.csv") %>% 
  clean_names()

main_index <- read_csv("../data/mainindex_11_09.csv")

newspaper_list <- read_csv("../data/newspaper_list.csv") 

clean_newspaper_list <- newspaper_list %>% 
  distinct(lccn, .keep_all = TRUE)

paper_list_joined <- main_index %>% 
  left_join(newspaper_list, by = c("sn" = "lccn")) %>% 
  left_join(state_crosswalk, by = c("state"))
  
paper_list_problems <- paper_list_joined %>% 
  filter(duplicated(file_id) | duplicated(file_id, fromLast = TRUE)) %>% 
  filter(is.na(newspaper_state_clean) | newspaper_state_clean == abbreviation) %>% 
  mutate(newspaper_state_clean = case_when(
    str_detect(newspaper, "N.D.") ~ "ND",
    str_detect(newspaper, "Ark") ~ "AR",
    str_detect(newspaper, "Utah") ~ "UT",
    str_detect(newspaper, "Chicago") ~ "IL",
    str_detect(newspaper, "New Haven") ~ "CT",
    str_detect(newspaper, "Fowler") ~ "MO",
    TRUE ~ newspaper_state_clean
  )) %>% 
  distinct(file_id, newspaper_state_clean, .keep_all = TRUE)
  
paper_list_good <- paper_list_joined %>% 
  filter(!(duplicated(file_id) | duplicated(file_id, fromLast = TRUE)))

full_list_clean <- paper_list_good %>% 
  rbind(paper_list_problems) %>%
  mutate(newspaper_state_clean = case_when(
    is.na(newspaper_state_clean) ~ abbreviation,
    TRUE ~ newspaper_state_clean)) %>% 
  mutate(newspaper_state_clean = case_when(
    str_detect(sn, "sn84037526") ~ "VI",
    str_detect(sn, "sn96080161|sn94055234") ~ "NE",
    str_detect(sn, "97071090") ~ "OR",
    TRUE ~ newspaper_state_clean
  ))

clean_check <- full_list_clean %>% 
  filter(is.na(newspaper_state_clean))

write_csv(full_list_clean, "../data/main_index_with_names_111323.csv")
```
...
```{r}
mob_full <- read_csv("../output/mob_full_11_15.csv")
mob_melt <- read_csv("../output/mob_melt_11_15.csv")
mob_three <- read_csv("../output/mob3_11_15.csv")

main_index <- read_csv("../data/main_index_with_names_111323.csv")
main_selected <- main_index %>% 
  select(sn, newspaper, newspaper_state_clean, date) %>% 
  distinct(sn, .keep_all = TRUE) %>% 
  mutate(newspaper_clean = str_squish(sub("\\..*$", "",  newspaper))) %>% 
  distinct(newspaper_clean, .keep_all = TRUE)

master_index <- read_csv("../data/master_article_index_10.19.csv")
master_selected <- master_index %>% 
  select(file_id2:date, file_id:filename) %>% 
  mutate(docname = gsub(".txt", "", filename))

mob_full_subset <- mob_full %>% 
  select(docname:other, sn, Newspaper_Region, file_id_old, year, state_lynch, miles, in_state, decade)

mob_master <- mob_full_subset %>% 
  left_join(master_selected, by = c("docname")) %>% 
  mutate(sn = case_when(
    !is.na(sn.y) ~ sn.y,
    TRUE ~ sn.x
  )) %>% 
  select(docname:other, Newspaper_Region, sn, year, state_lynch, miles, in_state, decade, filename) %>% 
  rename(url = url.x, date = date.x)

mob_master_easy <- mob_master %>% 
  filter(!is.na(sn)) %>% 
  left_join(main_selected, by = "sn") %>% 
  clean_names() %>% 
  rename(newspaper = newspaper_y, date = date_x) %>% 
  select(docname, newspaper, date, url:newspaper_region, sn, newspaper_state_clean, year, state_lynch, miles, in_state, decade, filename)

mm_filter <- mob_master %>%
  clean_names() %>% 
  filter(is.na(sn)) %>% 
  mutate(newspaper_clean = str_squish(gsub("\\d+|\\s*\\([^)]+\\)|\\.|-", "", newspaper))) %>% 
  mutate(newspaper_clean = case_when(
    newspaper_clean == "Cayton's weekly" ~ "Cayton&#39;s weekly",
    newspaper_clean == "St Johnsbury Caledonian" ~ "The Caledonian",
    is.na(newspaper_clean) ~ "Mineral Point tribune",
    TRUE ~ newspaper_clean
  )) %>% 
  left_join(main_selected, by = c("newspaper_clean")) %>% 
  rename(newspaper = newspaper.y, date = date.x, sn = sn.y) %>% 
  select(docname, newspaper, date, url:newspaper_region, sn, newspaper_state_clean, year, state_lynch, miles, in_state, decade, filename)

master_small <- main_index %>% 
  select(sn, newspaper, newspaper_state_clean) %>% 
  distinct(sn, .keep_all = TRUE)

all_mob_connected <- mob_master_easy %>% 
  rbind(mm_filter) %>% 
  mutate(newspaper_region = case_when(
    str_detect(newspaper_state_clean, "SC|TX|LA|TN|MS|AR|AL|GA|VA|FL|NC|MD|DE|WV|KY|OK|DC")~"South",
    str_detect(newspaper_state_clean, "MO|OH|IN|KS|MI|WI|MN|IA|IL|NE|ND|SD")~"Midwest",
    str_detect(newspaper_state_clean, "NY|NH|VT|MA|CT|RI|PA|NJ|ME")~"Northeast",
    str_detect(newspaper_state_clean, "CA|NV|OR|CO|MT|WA|ID|WY|UT|NM|AZ|AK|HI")~"West",
    TRUE~newspaper_state_clean
  )) %>% 
  mutate(newspaper_border = case_when(
    str_detect(newspaper_state_clean, "MD|DE|WV|KY|MO")~"Border",
    TRUE~"Not_Border"
  )) %>% 
  arrange(filename) %>% 
  left_join(master_small, by = c("sn")) %>% 
  rename(newspaper = newspaper.y, newspaper_state_clean = newspaper_state_clean.y) %>% 
  select(docname, newspaper, date, url:newspaper_region, sn, newspaper_state_clean, year, state_lynch, miles, in_state, decade, filename)

write_sheet(all_mob_connected, "https://docs.google.com/spreadsheets/d/1XG2um-mkZ0ta6kikMuQucydNOPV2POrWLPtxRTuBh0o/edit#gid=0", sheet = "all_mob_connected")
...
write_sheet(decade_trends, "https://docs.google.com/spreadsheets/d/1XG2um-mkZ0ta6kikMuQucydNOPV2POrWLPtxRTuBh0o/edit#gid=0", sheet = "decade_trends")
...
write_sheet(regional_trends, "https://docs.google.com/spreadsheets/d/1XG2um-mkZ0ta6kikMuQucydNOPV2POrWLPtxRTuBh0o/edit#gid=0", sheet = "regional_trends")
```
...
cooccurrence_table <- all_mob_connected %>% 
  group_by(hostile, justice, orderly) %>% 
  count() %>% 
  mutate(named = case_when(
    hostile == 1 & justice == 1 & orderly == 1 ~ "all",
    hostile == 1 & justice == 1 ~ "hostile, justice",
    hostile == 1 & orderly == 1 ~ "hostile, orderly",
    orderly == 1 & justice == 1 ~ "oderly, justice",
    orderly == 0 & justice == 0 & hostile == 0 ~ "none"
  )) %>% 
  filter(!is.na(named)) %>% 
  ungroup() %>% 
  select(named, n) %>% 
