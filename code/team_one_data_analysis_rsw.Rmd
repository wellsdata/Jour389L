...
all_mob_connected <- mob_master_easy %>% 
  ...
  mutate(newspaper_region = case_when(
    ...
    str_detect(newspaper_state_clean, "NY|NH|VT|MA|CT|RI|PA|NJ")~"Northeast",
...
write_sheet(all_mob_connected, "https://docs.google.com/spreadsheets/d/1XG2um-mkZ0ta6kikMuQucydNOPV2POrWLPtxRTuBh0o/edit#gid=0")
...
## Analyzing by Decade
```{r}
decade_hostile_trends <- all_mob_connected %>% 
  filter(!is.na(decade)) %>% 
  group_by(decade, hostile, justice) %>% 
  count() %>% 
  pivot_wider(names_from = decade, values_from = n) %>% 
  mutate(named = case_when(
    hostile == 1 & justice == 0 ~ "Hostile",
