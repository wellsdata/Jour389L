...
tolnay_filtered$date_seq <- lapply(tolnay_filtered$date_est, function(x) seq(as.Date(x), by = "day", length.out = 8))
tolnay_filtered <- unnest(tolnay_filtered, cols = c(date_seq))

# Create a sequence of dates for 15 days after date1
ga_news$date_seq <- lapply(ga_news$date, function(x) seq(as.Date(x), by = "day", length.out = 6))
