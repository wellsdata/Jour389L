...
tolnay_filtered$date_seq <- lapply(tolnay_filtered$date_est, function(x) seq(as.Date(x), by = "day", length.out = 10))
...
ga_tolnay_news <- tolnay_filtered %>% 
  inner_join(ga_news, by=c("date_seq"="date_seq")) %>% 
...
ga_tolnay_second <- tolnay_filtered %>% 
  anti_join(ga_tolnay_news, by = c("name", "lynch_county"))
