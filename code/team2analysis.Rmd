...
distance_by_decade <- lynch_geocoded_9.16 %>% 
  select(decade, miles) %>% 
  group_by(decade) %>% 
...
distance_by_paper <- lynch_geocoded_9.16 %>%
  select(newspaper_name1, newspaper_state_code, miles) %>%
  group_by(newspaper_name1, newspaper_state_code) %>%
  filter(!is.na(newspaper_name1)) %>%
  summarize(mean_miles = mean(miles, na.rm=TRUE)) %>%
  mutate(mean_miles = round(mean_miles, digits = 2)) %>%
  rename(newspaper_name = newspaper_name1, newspaper_state = newspaper_state_code) %>%
  arrange(desc(mean_miles))
...
distance_by_state <- lynch_geocoded_9.16 %>%
